...
* **Vacation days remaining:** _How many vacation days does this employee still have available for the remainder of the current calendar year. Each January 1st this value will be updated with the amount entered for "Vacation days per year". There is no carry over of unused vacation days in Work Tyme. We also use this value when sending out [time-off]({{< relref "/docs/time_off.md" >}}) requests to the employee's supervisor so that the supervisor is aware of how many vacation days the employee has remaining before they approve/deny the request. This value is allowed to go into the negative as we feel it is up to the supervisor and HR if they will to approve additional vacation days for an employee_
